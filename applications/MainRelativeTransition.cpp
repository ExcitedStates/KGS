...
*/
    ...
    regex intListPat("[[:digit:]]+(\\.[[:digit:]]+)?(\\+[[:digit:]]+(\\.[[:digit:]]+)?)*");
    //regex intervalPat("(\\-?[[:digit:]]+)\\-(\\-?[[:digit:]]+)");
    std::smatch sm;

    const string resiInput = options.relativeDistances;

    if( regex_match(resiInput, intListPat) ){
        vector<string> tokens = Util::split(resiInput, '+');
        for(const string& token: tokens) {
            double distance = std::atof(token.c_str());
            goal_distances.push_back(distance);
        }
    }else{
        throw std::runtime_error("KGS error: Selection::DoubleClause - Can't parse \""+options.relativeDistances+"\"");
    }
    ...
    /*if(options.gradient == 0)
        ...
        direction = new LSNrelativeDirection(resNetwork);
    else{
