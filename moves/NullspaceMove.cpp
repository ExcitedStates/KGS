...
gsl_vector* NullspaceMove::projectOnNullspace(Configuration* conf, gsl_vector* gradient, gsl_vector* ret)
{
  ...
  if( gradient->size > conf->getNullspace()->NumDOFs() ) {
    ...
    gsl_vector *to_proj_short = gsl_vector_calloc(conf->getNullspace()->NumDOFs());
    ...
    gsl_vector *after_proj_short = gsl_vector_calloc(conf->getNullspace()->NumDOFs());
